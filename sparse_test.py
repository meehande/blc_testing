# -*- coding: utf-8 -*-
"""
Created on Tue Mar 29 13:15:21 2016

@author: Deirdre Meehan
"""


import sparse_blc as blc
import blc as dblc
import numpy as np
...
#for ii in xrange(50):    
    #for i in xrange(10):
density = 0.8
W,L,Rsampled,Rmissing,a = dblc.sampleR(R_groups, density)
Rsampled = sps.lil_matrix(Rsampled)
Rmissing = sps.lil_matrix(Rmissing)
...
while ( (tries < n_tries) and (groups_age < group_convergence) ):
   ...
   # Lambda = blc.createLambda(P,Rsampled)#users per group rating item m - pxm - 
    Rtilde[Rtilde==np.inf] = 0 #change infinite values to 0    
    Lambda = Lambda.tocsr()    
...
#        #Rtilde and lambda will be updated on the beginning of the next iteration...
    tries +=1
    #group convergence
    existing_groups = np.sum(a.data,0) > 0 #find which groups are empty/full
